...
>>>>>>> 3735bff18be96e6bcba318900f201f2d5acd6533
...
<<<<<<< HEAD
  //Kept pizza sizes in percentage values to avoid a forced synchronous layout due to changing values between pixels and percents
  function changePizzaSizes(size) {
    ...
    //Changed querySelectorAll to get ElementByClassName
    //Put this access to the DOM into a variable outside the loop
...
<<<<<<< HEAD
//calculate the number of pizzas needed according to each screen
var rows = screen.height/225;
var cols = screen.width/250;
var numofpizza = rows * cols;
// Moves the sliding background pizzas based on scroll position
function updatePositions() {
  //Added two transform properties on style.css so the moving pizzas would be on their own layer.
  frame++;
  window.performance.mark("mark_start_frame");
  //moving variables out of the for loop to increase efficiency
  //getElementsByClass name instead of querySelectorAll to increase efficiency
  var items = document.getElementsByClassName('mover');
  var scrolling = document.body.scrollTop
  var phase;
  var i = 0;
  var sinx = (scrolling/1250);
  for (; i < numofpizza; i++) {
    phase = Math.sin(sinx + (i % 5));
=======
// Moves the sliding background pizzas based on scroll position
function updatePositions() {
  frame++;
...
<<<<<<< HEAD
  ...
  var s = 256;
  var elem;
//Changed the 200 pizzas (fixed value) to the variable numofpizza, previously calculated
  for (; i < numofpizza; i++) {
